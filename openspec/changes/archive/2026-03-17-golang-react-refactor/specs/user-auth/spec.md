...
### 需求：双令牌认证的用户登录
系统应使用邮箱和密码认证用户，颁发存储在 HttpOnly cookies 中的双令牌（访问和刷新）。

#### 场景：成功登录
- **当** 用户向 POST /api/v1/auth/login 提交正确的邮箱和密码
- **则** 系统设置 access_token cookie（HttpOnly，SameSite=Strict，15 分钟）
- **且** 系统设置 refresh_token cookie（HttpOnly，SameSite=Strict，7 天，Path=/api/v1/auth/refresh）
- **且** 系统在响应体中返回用户资料
...
#### 场景：过期或已撤销的刷新令牌
- **当** 客户端使���无效/过期/已撤销的刷新令牌调用 POST /api/v1/auth/refresh
- **则** 系统返回 401 Unauthorized
- **且** 系统清除两个令牌 cookies
