# RSS 解析规格

## 新增需求

### 需求：解析 RSS 2.0 订阅源
系统应解析 RSS 2.0 格式的订阅源并提取订阅源和文章数据。

#### 场景：解析有效的 RSS 2.0 订阅源
- **当** 系统解析有效的 RSS 2.0 订阅源
- **则** 系统提取订阅源 title、link、description、lastBuildDate
- **且** 系统提取文章的 title、link、description、pubDate、author、content:encoded

### 需求：解析 Atom 1.0 订阅源
系统应解析 Atom 1.0 格式的订阅源并提取订阅源和条目数据。

#### 场景：解析有效的 Atom 订阅源
- **当** 系统解析有效的 Atom 订阅源
- **则** 系统提取订阅源 title、link、subtitle、updated
- **且** 系统提取条目的 title、link、summary、content、published、updated、author

### 需求：优雅处理格式错误的订阅源
系统应在不崩溃的情况下处理格式错误或不完整的订阅源。

#### 场景：缺少可选字段
- **当** 订阅源缺少可选字段（description、author 等）
- **则** 系统成功解析，缺少的字段为空值/null

#### 场景：无效的 XML
- **当** 订阅源包含无效 XML
- **则** 系统返回解析错误而不崩溃

### 需求：订阅源文章去重
系统不应为同一订阅源条目创建重复文章。

#### 场景：相同 GUID 的文章
- **当** 订阅源包含该订阅源已存在 GUID 的文章
- **则** 系统跳过创建重复文章
- **且** 系统不更新现有文章

#### 场景：无 GUID 的文章使用 link
- **当** 订阅源文章无 GUID 但有 link
- **则** 系统使用 link 作为唯一标识符
- **且** 系统不为相同 link 创建重复

### 需求：提取订阅源 favicon
系统应尝试提取 favicon URL 用于订阅源显示。

#### 场景：Favicon 可用
- **当** 订阅源网站在 /favicon.ico 有 favicon
- **则** 系统将 favicon URL 存储为 image_url

#### 场景：Favicon 不可用
- **当** 订阅源网站无 favicon
- **则** 系统 image_url 存储为 null
- **且** 解析仍然成功

### 需求：清理文章内容
系统应存储原始内容但为显示提供清理后的摘要。

#### 场景：存储原始内容
- **当** 文章包含 HTML 内容
- **则** 系统在 content 字段存储原始 HTML

#### 场景：生成 description 摘要
- **当** 文章被创建
- **则** 系统生成纯文本 description（去除 HTML，截断至 200 字符）
